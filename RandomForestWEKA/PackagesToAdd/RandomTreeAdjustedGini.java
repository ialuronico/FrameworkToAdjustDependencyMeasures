 ...
 */
public class RandomTreeAdjustedGini extends AbstractClassifier implements OptionHandler,
  ...
  public static final int SPLITCRIT_Gini = 2;
  public static final int SPLITCRIT_SGini = 3;
  public static final int SPLITCRIT_AGiniAlpha = 4;
  
  public static final Tag [] TAGS_SPLITCRIT = {
    new Tag(SPLITCRIT_DEFAULT, "Default, Info gain as Standard Random Forests"),
    new Tag(SPLITCRIT_Gini, "Gini gain"),
    new Tag(SPLITCRIT_SGini, "Standardized Gini Gain"),
    new Tag(SPLITCRIT_AGiniAlpha, "AGini(alpha) - Adjustment for Ranking"),    
  };

  /** Splitting criterion used */
  protected int m_splitCrit = SPLITCRIT_DEFAULT;
  
  /** Alpha for AGini(alpha) */
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
  			"\tSplitting criterion.\n" +
  			"\t(Default = Info gain as Weka Random Tree))",
  			"C", 1, "-C <infogain | ginigain | sginigain | aginialphagain>"));      
    ...
    newVector.addElement(new Option(
        "\tSet alpha for AGini(alpha)\t(default 0.05)", "A", 1,
        "-A <alpha>"));
  ...
  public String[] getOptions() {
    ...
    if (m_splitCrit != SPLITCRIT_DEFAULT) {
      ...
      switch (m_splitCrit) {
	      ...
	      result.add("ginigain");
				break;
			case SPLITCRIT_SGini:
	      result.add("sginigain");
				break;
			case SPLITCRIT_AGiniAlpha:
	      result.add("aginialphagain");
	      break;
	    }
    }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      if (tmpStr.equals("infogain")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_DEFAULT, TAGS_SPLITCRIT));
			} else if (tmpStr.equals("ginigain")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_Gini, TAGS_SPLITCRIT));
			} else if (tmpStr.equals("sginigain")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_SGini, TAGS_SPLITCRIT));
			} else if (tmpStr.equals("aginialphagain")) {
				setSplitCrit(new SelectedTag(SPLITCRIT_AGiniAlpha, TAGS_SPLITCRIT));
			} else {
    ...
    if (tmpStr.length() != 0) {
      m_alpha = Double.parseDouble(tmpStr);
    } else {
      m_alpha = 0.05;
    }
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      trainVariance = RandomTreeAdjustedGini.singleVariance(classProbs[0], totalSumSquared,
  ...
  protected class Tree implements Serializable {
    ...
    public void backfitData(Instances data) throws Exception {
      ...
      double trainVariance = 0;
      if (data.classAttribute().isNumeric()) {
        trainVariance = RandomTreeAdjustedGini.singleVariance(classProbs[0],
    ...
    protected void backfitData(Instances data, double[] classProbs,
      ...
      if (data.classAttribute().isNumeric()) {
        ...
        priorVar = RandomTreeAdjustedGini.singleVariance(totalSum, totalSumSquared,
          totalSumOfWeights);
      }
      ...
      if (attribute.isNominal()) {
        ...
        for (int i = 0; i < indexOfFirstMissingValue; i++) {
          ...
          if (inst.value(att) > currSplit) {
            currVal = RandomTreeAdjustedGini.variance(currSums, currSumSquared,
              currSumOfWeights);
            if (currVal < bestVal) {
              bestVal = currVal;
     ...
     * 
     * The value depends on the selected criterion
     * 
     * @param dist the distributions
    ...
    protected double gain(double[][] dist, double priorVal) {
    	...
    	double splitCritVal = 0;
    	switch (m_splitCrit){
	 	  	case SPLITCRIT_Gini:
	 	  		splitCritVal = computeGini(dist);
	 	  		break;
	 	  	case SPLITCRIT_SGini:
	 	  		splitCritVal = computeSGini(dist);
	 	  		break;
	 	  	case SPLITCRIT_AGiniAlpha:
	 	  		splitCritVal = computeAGiniAlpha(dist); 
	 	  		break;
	 	  	default: // Information Gain
	 	  		splitCritVal = priorVal - ContingencyTables.entropyConditionedOnRows(dist);;
	 	  		break; 	  		
    	}
      return splitCritVal;
    }
    	
    protected double computeGini(double[][] dist){
      int r = dist.length;
    	int c = dist[0].length;
    	
    	double[] nY = new double [c];
    	double[] nX = new double[r];
    	int i,j;
    	double n = 0;
    	...
    	double GiniColumns = 1;
    
    	for (j = 0; j < c; j++) 
    		GiniColumns -= nY[j]*nY[j]/n/n;
	    ...
	    for (i = 0; i < r; i++) {
	      ...
	      if (nX[i] > 0){
	      for (j = 0; j < c; j++) 
					G += dist[i][j]*dist[i][j] /nX[i]/ nX[i];
	      }
	      GI_ += (1 - G)*nX[i];
	    }
	    
	    return  GiniColumns - GI_/n;
    	
    }
    
    protected double computeSGini(double[][] dist){
      int r = dist.length;
    	int c = dist[0].length;
	    ...
	    double GiniGain =  GiniColumns - GI_/n;
	    
	    // This is in order to avoid meaningless splits
	    if (!Utils.gr(GiniGain, 0))
	      return 0;
	    
	    // Compute r again if rows are empty
	    double R = 0;
	    ...
	    double EGI = (R - 1)/n*(GiniColumns);
	    double p2,p3,n1;
	    
	    p2 = 0;
	    for (j = 0; j < dist[0].length; j++) 
	         p2 = p2 + Math.pow(nY[j]/n,2);
	    
	    p3 = 0;
	    for (j = 0; j < dist[0].length; j++) 
	         p3 = p3 + Math.pow(nY[j]/n,3);
	    ...
	    double var = 1/n/n * ( (R-1) *  (2*p2 + 2*p2*p2 - 4*p3) + (n1 - 2*R/n + 1/n) * (-2*p2 -6*p2*p2 + 8*p3));
	        
	    // I don't want negative values because of the implementation of random forest
	    // Add a constant
	    double splitConst = 100;
	    
	    // Standardized Gini gain
	    double SGini = (GiniGain - EGI)/Math.sqrt(var);	    
	    return SGini + splitConst;

    }
    
    protected double computeAGiniAlpha(double[][] dist){
      int r = dist.length;
    	int c = dist[0].length;
	    ...
	    // Adjusted Gini gain alpha for ranking

	    // upper bound 
	    double lambdaCantelli = Math.sqrt( (1-m_alpha)/m_alpha);

	    // ~q(1-alpha)
	    double tildeQ = EGI + lambdaCantelli*Math.sqrt(var);

	    // Compute AGini(alpha)
	    double AGiniAlpha = GiniGain - tildeQ;
	    
	    // I don't want negative values because of the implementation of random forest
	    // Add a constant
	    double splitConst = 100;
	    	    
	    return AGiniAlpha + splitConst;
    }
  ...
  public static void main(String[] argv) {
    runClassifier(new RandomTreeAdjustedGini(), argv);
  }
}
