 ...
 */
public class Subagging
  extends RandomizableParallelIteratedSingleClassifierEnhancer 
  implements WeightedInstancesHandler, AdditionalMeasureProducer,
             TechnicalInformationHandler, PartitionGenerator, Aggregateable<Subagging> {
  ...
  protected synchronized Instances getTrainingSet(int iteration) throws Exception {
    ...
    */
    // Just want a random subset of instances
    
    bagData = new Instances(m_data,0);
    
  	LinkedList<Instance> list = new LinkedList<Instance>();
		LinkedList<Integer> pos = new LinkedList<Integer>();
		// Create a list of instances and position
		for (int i = 0; i < m_data.numInstances(); i++) {
			pos.add(i);
			list.add(m_data.instance(i));
		}
		// Sample an instance and put it in bagData
		while(list.size() > (double)(100 - m_BagSizePercent)/100*m_data.numInstances()){
  ...
  public static void main(String [] argv) {
    runClassifier(new Subagging(), argv);
  }
   ...
   */
  @Override
  public Subagging aggregate(Subagging toAggregate) throws Exception {
